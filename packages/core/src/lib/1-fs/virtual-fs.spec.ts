...
describe('Virtual Fs', () => {
  ...
  describe('create directories and navigation', () => {
    ...
    it('should get the parent', () => {
      const dir = fs.createDir('/project/src/main');
  ...
  describe('create and read files', () => {
    ...
    it('fails if it is a directory', () => {
      ...
      expect(() => fs.writeFile('/dir1', 'content 1')).toThrowError(
        'cannot write to file /dir1 because it is a directory'
      );
    });
  ...
  describe('search', () => {
    it('should find the index.ts in project directory', () => {
      fs.writeFile('/project/index.ts', 'hello');
